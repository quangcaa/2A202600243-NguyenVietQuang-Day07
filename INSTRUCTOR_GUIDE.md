...
- Mỗi sinh viên **tự mình** implement tất cả TODO trong template.py
- `Document` và `chunk_fixed_size` đã implement sẵn làm ví dụ
- Thứ tự gợi ý: `chunk_by_sentences` → `chunk_recursive` → `compute_similarity` → `compare_chunking_strategies` → `EmbeddingStore` → `KnowledgeBaseAgent`

**Vai trò giảng viên:**
- **Nhấn mạnh**: "Đây là phần cá nhân — mỗi người tự code"
- **Checkpoint 1 (1:00)**: "Ai đã pass TestChunkBySentences?" — giải thích nếu < 50%
...
**Vai trò giảng viên:**
- Khuyến khích mỗi người thử strategy khác nhau: "Một người thử fixed_size, một người thử recursive, một người thử custom"
